...
//             if (allPapers[i]._id.toString() !== research_paper_id.toString()) {
...
const checkLevelOne = async (req: any, res: Response) => {
    try {
        ...
        return res.json({ success: true, message: 'Level 1 Plagiarism report generated successfully.', data: plagiarismReport, mean: plagiarismReport.length > 0 ? sum / plagiarismReport.length : 0 });
    } catch (error:any) {
        // Handle other errors
